...
final class WPCM_Archive_Reader {
    ...
    const MIN_SLICE     = 4 * 1024 * 1024;
    const MAX_SLICE     = 256 * 1024 * 1024;
    ...
    public static function extract_step( $path, $extract_root, array $state, $time_budget, $memory_limit ) {
        ...
        $time_budget     = max( 1.0, min( 15.0, (float) $time_budget ) );
        $pending         = array();
        $output          = null;
            ...
            while ( ftell( $handle ) < $payload_length ) {
                if ( is_array( $current ) ) {
                    ...
                    if ( 'FEND' === $chunk_tag ) {
                        ...
                        if ( $completed_files >= 4000 || $pending_state_bytes >= 2 * MB_IN_BYTES ) {
                            break;
                        }
                        continue;
                    }
    ...
    private static function adapt_slice( $current, $duration, $processed, $memory_peak, $memory_limit, $time_budget, $growth_streak ) {
        ...
        } elseif ( $duration >= $time_budget * 0.95 ) {
            $next          = (int) floor( $current * 0.70 );
            $growth_streak = 0;
            $reason        = 'reduced_time';
        } elseif ( $processed >= (int) floor( $current * 0.90 ) && $duration < $time_budget * 0.62 && $memory_ratio < 0.65 ) {
            $growth_streak++;
            $reason = 'learning';
            if ( $growth_streak >= 2 ) {
